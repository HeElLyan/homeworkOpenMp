...
// reduction
// Приватная копия каждой перечисленной переменной инициализируется
// при входе в параллельную секцию в соответствии с указанным оператором
// (0 для оператора +). При выходе из параллельной секции из частично
// вычисленных значений вычисляется результирующее и передается в основной поток.
